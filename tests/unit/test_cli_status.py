...
def test_status_with_no_history(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_status_with_history(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("BOT_DB_PATH", str(tmp_path / "x.duckdb"))
    monkeypatch.setenv("BOT_SEC_USER_AGENT", "Tester t@x.com")
