...
def test_show_existing_ticker(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_show_missing_ticker_triggers_sec_import(
    ...
    def populate_then_return(conn: object, *, ticker: str, user_agent: str) -> IngestResult:
        import duckdb as dk

        assert isinstance(conn, dk.DuckDBPyConnection)
        conn.execute(
            "INSERT INTO companies (ticker, name, country, currency, source) VALUES (?, ?, ?, ?, ?)",
