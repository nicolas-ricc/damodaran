...
def test_parse_extracts_annual_revenue_and_net_income() -> None:
...
def test_parse_prefers_most_recent_filing_for_restated_year() -> None:
    result = parse_company_facts("AAPL", FAKE_FACTS)
    annual = {row["fiscal_year"]: row for row in result.annual}
    assert annual[2023]["revenue"] == 383285000000


def test_parse_extracts_quarterly() -> None:
    result = parse_company_facts("AAPL", FAKE_FACTS)
    q = [
        row for row in result.quarterly if row["fiscal_quarter"] == 2 and row["fiscal_year"] == 2024
    ]
    assert len(q) == 1
    assert q[0]["revenue"] == 90753000000


def test_parse_returns_filings_log_entries() -> None:
    result = parse_company_facts("AAPL", FAKE_FACTS)
