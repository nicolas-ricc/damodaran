...
-- Screener candidates — one row per (run_id, preset, ticker) shortlist entry.
-- ticker has a documented relationship to companies.ticker (no FK enforced in DuckDB).
CREATE TABLE IF NOT EXISTS screener_candidates (
    ...
    PRIMARY KEY (run_id, preset, ticker)
);
